...
interface TabUploadFromOtherFileStoreProps {
  remind?: string;
  remindColor?: string;
  modelName?: string;
  params?: Record<string, any>;
  onClose: () => void;
  onUploadFromOtherFileStoreComplete: (selectedFiles: any[], selectedRadio: string) => void;
  files: any[];
  oneFile?: boolean;
  limitFileExtension?: string
}

const TabUploadFromOtherFileStore: React.FC<TabUploadFromOtherFileStoreProps> = (props) => {
  ...
  const $_onPress = (item: any) => {
    if (!oneFile) {
      const index = selectedFiles.findIndex(file => file.id === item.id);
      if (index !== -1) {
  ...
  const $_submit = () => {
    onUploadFromOtherFileStoreComplete(selectedFiles, selectedRadio)
    onClose()
  }
        ...
        <View
          ...
          <WsPageIndex
            ...
            renderItem={({ item, index }) => {
              return (
                <LlFileCard001
                  checked={selectedFiles.some(file => item.id === file.id)}
                  item={item}
                  onPress={() => {
                    $_onPress(item)
                  }}
...
export default TabUploadFromOtherFileStore